...
struct cons_ctx_t {
  ...
  cons_ctx_t(std::vector<romulus::ReliableConnection *> conns,
      ...
      : conns_(std::move(conns)), proposed_(std::move(proposed)),
        laddr_(scratch), log_mat_(std::move(log_mat)) {}
};
...
struct replication_ctx_t {
  ...
  replication_ctx_t(
    ...
    romulus::LocalAddr laddr)
      : conns_mat_(std::move(conns_mat)), raddrs_mat_(std::move(raddrs_mat)),
        laddr_(laddr) {}
};
...
struct perm_ctx_t {
  ...
  perm_ctx_t(std::vector<std::vector<romulus::ReliableConnection *>> conns_mat,
             ...
             romulus::LocalAddr laddr)
      : conns_mat_(std::move(conns_mat)), laddr_(laddr),
...
struct fd_ctx_t {
  ...
  fd_ctx_t(std::vector<romulus::ReliableConnection *> conns,
           std::vector<romulus::RemoteAddr> raddrs, romulus::LocalAddr laddr)
      : conns_(std::move(conns)), raddrs_(std::move(raddrs)), laddr_(laddr) {}
};