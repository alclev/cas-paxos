...
#define INGEST_ARGS(args)                                                      \
  ...
  [[maybe_unused]] auto num_qps = args->uget(romulus::NUM_QP);                 \
  auto key_range = args->uget(romulus::KEY_RANGE);                             \
  [[maybe_unused]] auto num_shared_cq = args->uget(romulus::NUM_SHARED_CQ);    \
  [[maybe_unused]] auto outstanding_reqs = args->uget(romulus::OUTSTANDING_REQS);
