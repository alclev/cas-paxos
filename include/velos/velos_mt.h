 ...
 public:
  ...
  void Init(std::string_view dev_name, int dev_port,
    ...
    std::size_t remote_len =
        (2 * scratch_len) + proposal_len + log_len + leader_len;
    raw_ =
    ...
    memblock_.RegisterMemRegion(kScratchPrepareId, 0, scratch_len);
    memblock_.RegisterMemRegion(kScratchPromiseId, scratch_len, scratch_len);

    memblock_.RegisterMemRegion(kProposedRegionId, 2 * scratch_len,
                                proposal_len);
    memblock_.RegisterMemRegion(kLogRegionId, (2 * scratch_len + proposal_len),
                                log_len);
    memblock_.RegisterMemRegion(
        kLeaderRegionId, 2 * scratch_len + proposal_len + log_len, leader_len);
  ...
  void Propose([[maybe_unused]] uint32_t len, [[maybe_unused]] uint8_t* buf) {
    Value v;
#ifndef STANDALONE
    v.SetId(host_id_);
  ...
  State* Prepare() {
    State* curr_proposal = &proposed_state_[prepare_offset_];
    ...
    while (done_count < quorum_) {
      ...
      for (uint32_t i = 0; i < system_size_; ++i) {
        ...
        c->log_raddr.addr_info.offset = prepare_offset_ * kSlotSize;
    ...
    ROMULUS_DEBUG("Prepared slot: log_offset={}, state={}", prepare_offset_,
                  curr_proposal->ToString());

    if (prepare_offset_ < capacity_) {
      prepare_offset_++;
    }
  ...
  bool Promise(Value v) {
    if (promise_offset_ >= prepare_offset_) {
      DoBackoff(std::chrono::nanoseconds(std::rand() % kMaxStartingBackoff));
      return false;
    }
