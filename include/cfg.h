...
constexpr const char* OUTSTANDING_REQS = "--outstanding-reqs";
...
inline auto EXTRA_ARGS = {
  ...
  U64_ARG_OPT(CAPACITY, "Capacity of the replicated log.", (1ULL << 21)),
  U64_ARG_OPT(BUF_SIZE, "Buffer size for remote writes.", 64),
  U64_ARG_OPT(SLEEP, "Sleep interval between proposals in ms", 0),
  BOOL_ARG_OPT(STABLE_LEADER, "If true, only a single node proposes commands."),
  ...
  U64_ARG_OPT(OUTSTANDING_REQS, "Pipeline depth.", 0)
};
